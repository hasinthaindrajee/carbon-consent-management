...
public class SQLConstants {
    ...
    public static final String INSERT_PURPOSE_ENTITY_ASSOC_SQL = "INSERT INTO CM_PURPOSE_EXT_ENTITY_ASSOC (PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE, TENANT_ID) values (?, ?, ?, ?)";

    public static final String DELETE_PURPOSE_ENTITY_ASSOC_SQL = "DELETE FROM CM_PURPOSE_EXT_ENTITY_ASSOC WHERE " +
            "ID = ? AND TENANT_ID = ?";

    public static final String DELETE_PURPOSE_ENTITY_ASSOC_BY_PURPOSE_ID_SQL = "DELETE FROM " +
            "CM_PURPOSE_EXT_ENTITY_ASSOC WHERE " +
            "PURPOSE_ID = ?";

    public static final String GET_PURPOSE_ENTITY_ASSOC_SQL = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM CM_PURPOSE_EXT_ENTITY_ASSOC WHERE " +
            "ID = ? AND TENANT_ID = ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_PURPOSE_ID = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM CM_PURPOSE_EXT_ENTITY_ASSOC WHERE " +
            "PURPOSE_ID = ? AND EXTERNAL_ENTITY_NAME " +
            "LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_PURPOSE_ID_DB2 = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM (SELECT ROW_NUMBER() OVER (ORDER BY ID) AS rn, p.*  " +
            "FROM CM_PURPOSE_EXT_ENTITY_ASSOC AS p) WHERE " +
            "PURPOSE_ID = ? AND EXTERNAL_ENTITY_NAME " +
            "LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? rn BETWEEN ? AND ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_PURPOSE_ID_INFORMIX = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM CM_PURPOSE_EXT_ENTITY_ASSOC WHERE " +
            "PURPOSE_ID = ? AND EXTERNAL_ENTITY_NAME " +
            "LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_PURPOSE_ID_ORACLE = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM (SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, " +
            "TENANT_ID, rownum AS rnum FROM (SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, TENANT_ID" +
            " FROM CM_PURPOSE_EXT_ENTITY_ASSOC ORDER BY ID) WHERE PURPOSE_ID =? AND EXTERNAL_ENTITY_NAME LIKE ? " +
            "AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? AND rownum <= ?) WHERE rnum > ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_PURPOSE_ID_MSSQL = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM (SELECT ID, PURPOSE_ID, EXTERNAL_ENTITY_NAME, " +
            "EXTERNAL_ENTITY_TYPE, ROW_NUMBER() OVER (ORDER BY ID) AS RowNum FROM CM_PURPOSE_EXT_ENTITY_ASSOC) AS P " +
            "WHERE PURPOSE_ID =? AND EXTERNAL_ENTITY_NAME LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND P.TENANT_ID = ?" +
            "AND P.RowNum BETWEEN ? AND ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_ENTITY_NAME_TYPE = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM CM_PURPOSE_EXT_ENTITY_ASSOC WHERE EXTERNAL_ENTITY_NAME " +
            "LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_ENTITY_NAME_TYPE_DB2 = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM (SELECT ROW_NUMBER() OVER (ORDER BY ID) AS rn, p.*  " +
            "FROM CM_PURPOSE_EXT_ENTITY_ASSOC AS p) WHERE EXTERNAL_ENTITY_NAME " +
            "LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? rn BETWEEN ? AND ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_ENTITY_NAME_TYPE_INFORMIX = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM CM_PURPOSE_EXT_ENTITY_ASSOC WHERE AND " +
            "EXTERNAL_ENTITY_NAME " +
            "LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_ENTITY_NAME_TYPE_ORACLE = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM (SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, " +
            "TENANT_ID, rownum AS rnum FROM (SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, TENANT_ID" +
            " FROM CM_PURPOSE_EXT_ENTITY_ASSOC ORDER BY ID) WHERE EXTERNAL_ENTITY_NAME LIKE ? " +
            "AND EXTERNAL_ENTITY_TYPE LIKE ? AND TENANT_ID = ? AND rownum <= ?) WHERE rnum > ?";

    public static final String SEARCH_PURPOSE_ENTITY_ASSOCIATIONS_BY_ENTITY_NAME_TYPE_MSSQL = "SELECT ID, PURPOSE_ID, " +
            "EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE FROM (SELECT ID, PURPOSE_ID, EXTERNAL_ENTITY_NAME, " +
            "EXTERNAL_ENTITY_TYPE, ROW_NUMBER() OVER (ORDER BY ID) AS RowNum FROM CM_PURPOSE_EXT_ENTITY_ASSOC) AS P " +
            "WHERE EXTERNAL_ENTITY_NAME LIKE ? AND EXTERNAL_ENTITY_TYPE LIKE ? AND P.TENANT_ID = ?" +
            "AND P.RowNum BETWEEN ? AND ?";
