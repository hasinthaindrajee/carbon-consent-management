...
CREATE TABLE CM_PURPOSE_EXT_ENTITY_ASSOC (
  ...
  TENANT_ID             INTEGER DEFAULT -1234 NOT NULL,
  CONSTRAINT PURPOSE_EXT_ENTITY_ASSOC_CONSTRAINT UNIQUE (PURPOSE_ID, EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE),
  PRIMARY KEY (ID)
)
/
CREATE SEQUENCE CM_PURPOSE_EXT_ENTITY_ASSOC_SEQ
  START WITH 1
  INCREMENT BY 1 NOCACHE
/
CREATE TRIGGER CM_PURPOSE_EXT_ENTITY_ASSOC_TRIGGER NO CASCADE BEFORE INSERT ON CM_PURPOSE_EXT_ENTITY_ASSOC
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.ID)
       = (NEXTVAL FOR CM_PURPOSE_EXT_ENTITY_ASSOC_SEQ);
