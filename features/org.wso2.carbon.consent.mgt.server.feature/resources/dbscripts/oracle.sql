...
CREATE OR REPLACE TRIGGER CM_PURPOSE_TRIG
  ...
  END;
/
CREATE TABLE CM_PURPOSE_EXT_ENTITY_ASSOC (
  ID                    INTEGER,
  PURPOSE_ID            VARCHAR(255) NOT NULL,
  EXTERNAL_ENTITY_NAME  VARCHAR(255) NOT NULL,
  EXTERNAL_ENTITY_TYPE  VARCHAR(255) NOT NULL,
  TENANT_ID   INTEGER DEFAULT -1234,
  CONSTRAINT CM_PURPOSE_ENT_CONSTRAINT UNIQUE (PURPOSE_ID, EXTERNAL_ENTITY_NAME, EXTERNAL_ENTITY_TYPE),
  PRIMARY KEY (ID)
)
/
CREATE SEQUENCE CM_PURPOSE_ENTITY_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER CM_PURPOSE_ENTITY_TRIG
  BEFORE INSERT
  ON CM_PURPOSE_EXT_ENTITY_ASSOC
  REFERENCING NEW AS NEW
  FOR EACH ROW
  BEGIN
    SELECT CM_PURPOSE_ENTITY_SEQ.nextval INTO :NEW.ID FROM dual;
  END;
/
